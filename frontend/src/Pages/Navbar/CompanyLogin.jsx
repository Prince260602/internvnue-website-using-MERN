...
const handleInput = (e) => {
  ...
  const handleSubmit = (e) => {
   ...
   if(errors.email === "" && errors.password === "" ) {
      ...
      axiosInstance.post('http://localhost:8800/api/user/login', values).then(res => {
        console.log(values)
          console.log(res)
          navigate("/")
      }).catch(err =>{
   ...
   <>
    <div className="main min-h-screen w-screen flex flex-col md:flex-row-reverse mx-0 my-0">
         ...
         <img className="h-8 mb-8 mt-6" src="https://dersyb7nfifdf.cloudfront.net/public/assets/logointervue.png" alt="Logo" />
               ...
               <h1 className="mt-4 text-2xl font-semibold md:text-2xl">Login as Organisation</h1>
               ...
               <form className="mt-8 flex flex-col gap-3" onSubmit={handleSubmit}>
                  <div className='flex flex-col gap-5 justify-center align-center items-center mb-2'>
                     <div className='flex flex-col gap-2'>
                        <input type="text" placeholder='Work email' className='bg-gray-200 p-2 w-80 text-lg rounded-lg'  onChange={handleInput} value={values.email} name="email"/>
                        {errors.email && <span className='text-red-500 text-left'> {errors.email} </span>}
                     </div>
                     <div className='flex flex-col gap-2'>
                        <input type="text" placeholder='Password' className='bg-gray-200 p-2 w-80 text-lg rounded-lg' onChange={handleInput} value={values.password} name="password"/>
                        {errors.password && <span className='text-red-500 tex-left'> {errors.password} </span>}
                     </div>  
                  </div>
                  <div className='flex items-center gap-2 justify-center mb-5 cursor-pointer'>
                     <a href="https://www.intervue.io/become-an-interview-engineer">Login as interviewer instead </a>
                     <img src="https://www.svgrepo.com/show/28675/right-arrow.svg" alt="logo"  className='h-5 w-5 ' /> 
                  </div>
                  <div className='flex gap-2 p-2 text-white bg-black text-lg items-center justify-center mx-auto align-center w-80 mb-5'>
                     <button className='cursor-pointer ' type="submit">Login with email</button>
                     <img src="https://www.svgrepo.com/show/28675/right-arrow.svg" alt="logo"  className='h-5 w-5  ' /> 
                  </div>
               <Link to="/CompanyForgetpassword"><p className='underline mb-5 cursor-pointer flex justify-center'>Forgot Password?</p></Link>
                  <div className='flex flex-col mb-5 flex justify-center items-center'>
                     <span className='text-lg'>No account? Join now!</span>
                     <Link to="/CompanySignup"><span className='text-green-600 font-bold text-lg'>Create an account</span></Link>
                  </div>
                  <div className='flex flex-col gap-2 items-center justify-center'>
                     <span>This site is protected by the Intervue's</span>
         ...
         <div className="second w-screen flex-grow text-start px-6 bg-stone-100 py-10 md:w-1/2 md:h-screen md:sticky md:top-0 md:flex md:flex-col md:justify-center md:px-20">
          ...
          <Link to="/CompanyLogin" className="flex gap-3  items-center text-sky-600 font-semibold hover:underline">
